...
uint256 hashGenesisBlock("0x2cadd7467aa4ca55a8be084aa8779da4f250981cbd7a7e377a4502bb58e5578b");
...
{
    // CampusCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // CampusCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xd7, 0xcf, 0xa4, 0xe6 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CampusCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CampusCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CampusCoinMiner(CWallet *pwallet)
{
    printf("CampusCoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running CampusCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CampusCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CampusCoinMiner, pwallet));
}
